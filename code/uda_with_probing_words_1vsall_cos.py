...
assert args.cos_clsifer is True, 'ablation study for cosine classifier'
...
def train(labeled_trainloader, unlabeled_trainloader, model, optimizer, criterion, epoch, n_labels, train_aug,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        with torch.no_grad():
            ...
            assert args.prob_loss_func == 'bce', 'bce function is required'
            if args.prob_loss_func == 'bce':
                # prob_proba_orig = torch.sigmoid(prob_logits_orig)
                prob_proba_orig = torch.clamp(prob_logits_orig, min=0)
...
class SemiLoss(object):
    ...
    def __call__(self, outputs_x, targets_x, prob_logits_x, outputs_u=None, targets_u=None, prob_logits_u=None,
        ...
        if loss_type == '1-vs-all':
            ...
            loss_prob_x = self.bce_loss(torch.clamp(prob_logits_x.view(-1), min=0.0), targets_x_1hot.view(-1).cuda()).mean()
        ...
        if loss_u_mask.sum() > 0:
            ...
            if loss_type == '1-vs-all':
                ...
                loss_prob_u1 = self.bce_loss(torch.clamp(prob_logits_u1.view(-1), min=0.0),
                                             torch.clamp(targets_u_prob, min=0.0).view(-1))
                loss_prob_u1 = (loss_prob_u1 * loss_prob_mask).sum() / loss_prob_mask.sum()
                loss_prob_u2 = self.bce_loss(torch.clamp(prob_logits_u2.view(-1), min=0.0),
                                             torch.clamp(targets_u_prob, min=0.0).view(-1))
