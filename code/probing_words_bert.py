    ...
    def forward(self, input_ids, token_type_ids=None, position_ids=None, probing_words_list=None):
        ...
        if probing_words_list is not None:
            for cls_id in range(len(probing_words_list)):
                ...
                if global_file.args.parallel_prob:
                    if cls_id == len(probing_words_list) - 1:
                        # sep token
                        cur_cls_avg_embedding = cur_cls_words_embeddings.mean(1, keepdim=True)
                    else:
                        ...
                        #     random_tensor = torch.tensor(np.random.binomial(n=1, p=global_file.args.dropout_prob, size=(num_seq, num_prob))).cuda()
                        #     random_tensor = random_tensor.unsqueeze(-1).repeat(1, 1, hidden_size)
                        #     cur_cls_avg_embedding = (cur_cls_words_embeddings * random_tensor).sum(1, keepdim=True) / random_tensor.sum(1, keepdim=True)
                        # else:
                        cur_cls_avg_embedding = cur_cls_words_embeddings.mean(1, keepdim=True)
                else:
...
class ClassificationBertWithProbingWord(nn.Module):
    def __init__(self, num_labels, model='bert-base-uncased'):
        super().__init__()
        config = BertConfig.from_pretrained('./cache/pytorch_transformers/4dad0251492946e18ac39290fcfe91b89d370fee250efe9521476438fe8ca185.7156163d5fdc189c3016baca0775ffce230789d7fa2a42ef516483e4ca884517', output_hidden_states=True, output_attentions=True,
                                            cache_dir=None, force_download=False)
        self.bert = BertModelAvgPrb.from_pretrained('./cache/pytorch_transformers/aa1ef1aede4482d0dbcd4d52baad8ae300e60902e88fcb0bebdec09afd232066.36ca03ab34a1a5d5fa7bc3d03d55c4fa650fed07220e2eeebc06ce58d0e9a157', config=config)
    ...
    def forward(self, token_id, text_len, probing_words_list=None, use_cls_sep=True, output_attention=False, split_flag=False, cos_dist=False):
        ...
        if probing_words_list is None:
            ...
            # ipdb.set_trace()
            # print(probing_words_list)
            added_len = len(probing_words_list)
            len_range = torch.arange(seq_length + added_len, device=token_id.device,
                                     dtype=text_len.dtype).expand(num_seq, seq_length + added_len)
            mask1 = (len_range < text_len.unsqueeze(-1)).long()
            mask1[:, -added_len:] += 1
        ...
        if probing_words_list is None:
            ...
            # ============ gap on whole sequence ============
            # seq_gap_fea = torch.mean(out[:, :-added_len], 1)
            # seq_logits = self.classifier(seq_gap_fea)
            # ============ gap on text (w/ cls&sep token feature, w/o padding) ============
            # mask1[:, -added_len:] -= 1
            # seq_fea = out * mask1.unsequeeze(-1).expand(num_seq, seq_length + added_len, self.bert.config.hidden_size)
            # seq_gap_fea = torch.sum(seq_fea, 1) / mask1.sum(1, keepdim=True)
            #  ============ gap on text (without padding and cls&sep token) ============
            if use_cls_sep:
                mask1[:, -added_len-1:] -= 1
                mask1[:, 0] -= 1
            else:
                mask1[:, -added_len] -= 1
            seq_fea = out[0] * mask1.unsqueeze(-1).expand(num_seq, seq_length + added_len, self.bert.config.hidden_size)
            seq_gap_fea = torch.sum(seq_fea, 1) / mask1.sum(1, keepdim=True)
            seq_logits = self.classifier(seq_gap_fea)

            prob_head_logits = list()
            for cls_id in range(added_len-1):
                if cos_dist:
                    assert not global_file.args.multiple_sep, 'Do not use multiple sep'
                    cos = nn.CosineSimilarity(dim=1, eps=1e-6)
                    prob_head_logits.append(cos(seq_gap_fea, out[0][:, cls_id - added_len]))
                else:
                    if global_file.args.multiple_sep:
                        cur_match_fea = torch.cat((seq_gap_fea, out[0][:, cls_id*2 - added_len]), 1)
                    else:
                        cur_match_fea = torch.cat((seq_gap_fea, out[0][:, cls_id - added_len]), 1)
                    prob_head_logits.append(self.prob_match_cls(cur_match_fea))

            prob_head_logits = torch.stack(prob_head_logits, 1).squeeze()
            if len(prob_head_logits.size()) == 1 and not split_flag:
