...
def main():
    ...
    if args.eval:
        ...
        # cls_token_ids = None
        # val_loss, val_acc = validate(val_loader, model, ce_criterion, epoch, mode='Valid Stats', cls_token_ids=cls_token_ids)
        # logger.info("epoch {}, val acc {}, val_loss {}".format(epoch, val_acc, val_loss))
        #
        # test_loss, test_acc = validate(test_loader, model, ce_criterion, epoch, mode='Test Stats ', cls_token_ids=cls_token_ids)
        # logger.info("epoch {}, test acc {},test loss {}".format(epoch, test_acc, test_loss))
        # writer.add_scalar('test/loss', test_loss, epoch)
        # writer.add_scalar('test/acc', test_acc, epoch)
        unlabeled_trainloader = Data.DataLoader(dataset=train_unlabeled_set, batch_size=300, drop_last=False)
...
def train(labeled_trainloader, labeled_train_iter, unlabeled_trainloader, unlabeled_train_iter, model, optimizer, criterion, epoch, n_labels, train_aug, cls_token_ids):
    model.train()
    ...
    for batch_idx in range(args.val_iteration):
        ...
        outputs_x, prob_logits_x = model(inputs_x, inputs_x_length, probing_words_list=cls_token_ids, use_cls_sep=args.add_cls_sep)
        # ipdb.set_trace()
        ...
        with torch.no_grad():
            orig_y_pred, prob_logits_orig = model(inputs_ori, length_ori, probing_words_list=cls_token_ids, use_cls_sep=args.add_cls_sep)
        ...
        aug_y_pred, prob_logits_aug = model(inputs_u, length_u, probing_words_list=cls_token_ids, use_cls_sep=args.add_cls_sep)
        aug_y_pred2, prob_logits_aug2 = model(inputs_u2, length_u2, probing_words_list=cls_token_ids, use_cls_sep=args.add_cls_sep)

        loss = criterion(outputs_x, targets_x, prob_logits_x, (aug_y_pred, aug_y_pred2), orig_y_pred,
...
def eval_acc(model, unlabeled_trainloader, epoch, cls_token_ids):
    ...
    pl_prob_num = 0
    sample_num = 0
    for batch_idx, ((inputs_u, inputs_u2, inputs_ori), (length_u, length_u2, length_ori), idx_u, targets_u_ori) in enumerate(unlabeled_trainloader):
        inputs_ori = inputs_ori.cuda()
        length_ori = length_ori.cuda()
        targets_u_ori = targets_u_ori.cuda()
        with torch.no_grad():
            orig_y_pred, prob_logits_orig = model(inputs_ori, length_ori, probing_words_list=cls_token_ids,
...
class SemiLoss(object):
    ...
    def __call__(self, outputs_x, targets_x, prob_logits_x, outputs_u=None, targets_u=None, prob_logits_u=None, loss_u_mask=None, epoch=None, label_smooth=False):
        if label_smooth:
            loss = self.loss_label_smoothing(outputs_x.view(-1, self.n_labels), targets_x.view(-1))
            loss_prob_x = self.loss_label_smoothing(prob_logits_x.view(-1, self.n_labels), targets_x.view(-1))
            pass
        else:
            ...
            if args.tsa:
                ...
                targets_x_1hot = torch.zeros(outputs_x.shape[0], self.n_labels).scatter_(1, targets_x.cpu().view(-1, 1), 1).cuda()
