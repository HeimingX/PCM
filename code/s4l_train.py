...
def main():
    ...
    for epoch in range(args.epochs):
        # mlm_epoch, mlm_iter = train(labeled_trainloader, mlm_iter, model, optimizer, optimizer_mlm, scheduler,
        #                             criterion, epoch, mlm_epoch, tokenizer)
        #
        # # save model
        # ckpt = {
        #     'epoch': epoch,
        ...
        # }
        # filename = 'checkpoint.s4l_ep{}.ckpt'.format(epoch)
        # checkpoint_path = os.path.join('./ckpt/s4l/', filename)
...
def train(labeled_trainloader, mlm_iter, model, optimizer, optimizer_mlm, scheduler, criterion, epoch, mlm_epoch, tokenizer):
    ...
    for batch_idx in range(args.val_iteration):
        ...
        # mlm training
        outputs = model(input_ids, token_type_ids=segment_ids, attention_mask=input_mask, masked_lm_labels=lm_label_ids,
        ...
        print('epoch {}, step {}, loss {}, mlm loss {}'.format(epoch, batch_idx, loss.item(), mlm_loss.item()))
        train_time += time.time() - end
        end = time.time()
