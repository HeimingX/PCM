...
parser.add_argument('--prob_file_name', default='uda_top5_noStopWords_probWrods_ep1.npy')
parser.add_argument('--init_best_acc', default=0.6539, type=float, help='yahoo answers:uda(0.6539)')
...
parser.add_argument('--weighted_prob_word', action="store_true", default=False)
parser.add_argument('--weighted_type', default='softmax', type=str, help='weighted prob function type, reluNorm')
parser.add_argument('--parallel_prob', default=False, action="store_true")
parser.add_argument('--dropout_prob', default=0.5, type=float, help='dropout probing words')
parser.add_argument('--wProb', default=1.0, type=float)
parser.add_argument('--prob_loss_func', default='bce', type=str, help='probing words loss function, bce, ce')
...
parser.add_argument('--feed-prob', action="store_true", default=True)
...
def main(gpu):
    ...
    labeled_sampler = torch.utils.data.distributed.DistributedSampler(train_labeled_set, num_replicas=args.world_size,
                                                                      rank=rank)
    ul_sampler = torch.utils.data.distributed.DistributedSampler(train_unlabeled_set, num_replicas=args.world_size,
                                                                 rank=rank)
    val_sampler = torch.utils.data.distributed.DistributedSampler(val_set, num_replicas=args.world_size,
                                                                  rank=rank)
    ...
    labeled_trainloader = Data.DataLoader(dataset=train_labeled_set, batch_size=args.batch_size, shuffle=True,
                                          drop_last=True, sampler=labeled_sampler)
    unlabeled_trainloader = Data.DataLoader(dataset=train_unlabeled_set, batch_size=args.batch_size_u, shuffle=True,
                                            drop_last=True, sampler=ul_sampler)
    val_loader = Data.DataLoader(dataset=val_set, batch_size=128, shuffle=False, sampler=val_sampler)
    test_loader = Data.DataLoader(dataset=test_set, batch_size=128, shuffle=False, sampler=test_sampler)

    cls_token_ids, best_acc = init_probing_words(tokenizer)

    model = ClassificationBertWithProbingWord(n_labels, model=args.model).cuda(gpu)
    ...
    if args.eval:
        ...
        # ipdb.set_trace()
        #
        # unlabeled_trainloader = Data.DataLoader(dataset=train_unlabeled_set, batch_size=128, drop_last=False)
        # pl_all, prob_all = eval_acc(model, unlabeled_trainloader, epoch, cls_token_ids)
        # pl_all /= args.T
        # pl_proba = torch.softmax(pl_all, 1)
        # pl_val, pl_idx = torch.max(pl_proba, 1)
        # pl_mask = pl_val >= args.confid
        # prob_masked = prob_all[pl_mask]
        # prob_masked = torch.sigmoid(prob_masked)
        # u_l = train_unlabeled_set.labels
        # correct_mask = pl_idx[pl_mask].cpu().numpy() == u_l[pl_mask.cpu().numpy()]
        # false_mask = ~correct_mask
        #
        # correct_prob_proba = prob_masked[correct_mask].max(1)[0].mean()
        # false_prob_proba = prob_masked[false_mask].max(1)[0].mean()
        # ipdb.set_trace()
        ...
        #     checkpoint = torch.load(checkpoint_path)
        #     model.load_state_dict(checkpoint['state_dict'])
        #     eval_acc(model, unlabeled_trainloader, epoch, cls_token_ids)
        return
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        labeled_train_iter, unlabeled_train_iter = train(labeled_trainloader, labeled_train_iter, unlabeled_trainloader,
                                                         ...
                                                         n_labels, args.train_aug, cls_token_ids, gpu)
        if gpu == 0:
            # save model
            ckpt = {
                'epoch': epoch,
        ...
        val_loss, val_acc = validate(val_loader, model, ce_criterion, epoch, mode='Valid Stats',
                                     cls_token_ids=cls_token_ids, gpu=gpu)
        if gpu == 0:
            logger.info("epoch {}, val acc {}, val_loss {}".format(epoch, val_acc, val_loss))
        ...
        if val_acc >= best_acc:
            ...
            if args.prob_word_type == 'dynamic' and gpu == 0:
                # update attended tokens
...
def init_probing_words(tokenizer, gpu):
    cls_token_ids = list()
    if args.prob_word_type == 'dynamic':
        # cls_token_ids = None
        if args.prob_file_name == 'cls_names':
            ...
            cls_wise_attended_tokens = [
                ['bad', 'boring', 'awful', 'worst', 'terrible', 'poor', 'stupid', 'horrible', 'dull', 'mess', 'crap',
                 'annoying', 'cheap', 'fails', 'trash', 'pathetic', 'lame', 'lack', 'weak', 'silly', 'waste', 'poorly',
                 'ridiculous', 'little', 'garbage', 'amateur', 'wasted', 'disappointing', 'painful', 'pointless',
                 'badly', 'gore', 'irritating', 'fake', 'sick', 'stupidity', 'dumb', 'sucks', 'dreadful', 'stink',
                 'lacks', 'disgusting', 'rubbish', 'worse', 'nonsense', 'sucked', 'failure', 'idiots', 'hardly', 'hate',
                 'slash', 'failed', 'horror', 'worthless', 'bored', 'disaster', 'ugly', 'unfortunately', 'barely',
                 'embarrassing', 'boredom', 'sad', 'endless', 'idiot', 'useless', 'disappointment', 'hideous', 'fail',
                 'torture', 'unbelievable', 'bland', 'suck', 'flop', 'confusing', 'holes', 'slow', 'low', 'racist',
                 'shame', 'hell', 'monster', 'cheated', 'gross', 'junk', 'devoid', 'ted', 'flawed', 'sorry',
                 'confusion', 'bomb', 'horribly', 'lacked', 'lacking', 'clumsy', 'disgrace', 'twisted', 'disappointed',
                 'pain', 'bore', 'rotten', 'nasty', 'ruined', 'misery', 'racism', 'absurd', 'tired', 'frustrating',
                 'lunatic', 'flaws', 'meaningless', 'sloppy', 'biased'],
                ['wonderful', 'excellent', 'fine', 'fantastic', 'perfect', 'superb', 'beautiful', 'terrific', 'amazing',
                 'enjoyable', 'gem', 'brilliant', 'funny', 'masterpiece', 'favorite', 'perfection', 'definitely',
                 'finest', 'delightful', 'marvelous', 'nice', 'stunning', 'outstanding', 'incredible', 'comedy',
                 'completely', 'truly', 'enjoy', 'magnificent', 'surprisingly', 'recommended', 'lovely', 'powerful',
                 'compelling', 'solid', 'splendid', 'pleasant', 'perfectly', 'succeeds', 'beautifully', 'classics',
                 'intense', 'reasonably', 'fabulous', 'rare', 'strong', 'favourite', 'adaptation', 'enjoyed', 'love',
                 'flawless', 'gorgeous', 'delicious', 'explores', 'worth', 'amusing', 'dazzling', 'highly', 'marvel',
                 'impressive', 'charm', 'surely', 'sweetly', 'entertainment', 'clever', 'loved', 'awesome',
                 'production',
                 'flair', 'extremely', 'recommend', 'pass', 'fair', 'pleasure', 'favorites', 'expects', 'spectacular',
                 'beauty', 'power', 'stellar', 'straightforward', 'thanking', 'luck', 'elegance', 'laughs', 'adapting',
                 'awards', 'real', 'satisfying', 'polished']]
            best_acc = args.init_best_acc
    # else:
    elif args.prob_word_type == 'mixtext' or args.prob_word_type == 'uda':
        # cls_wise_attended_tokens = CLS_WISE_ATTENTED_TOKENS
        cls_wise_attended_tokens = np.load(args.data_path + args.prob_file_name, allow_pickle=True)
        cls_wise_attended_tokens = [list(cur_cls_token) for cur_cls_token in cls_wise_attended_tokens]
        # cls_wise_attended_tokens = get_class_names(args.data_path)
        best_acc = 0
    ...
    if args.add_cls_sep:
        ...
        # cls_wise_attended_tokens.append(['[SEP]'] * n_gpu)
        cls_wise_attended_tokens.append(['[SEP]'])
    for cls_tokens in cls_wise_attended_tokens:
        if args.prob_word_num > 0:
            cls_tokens = cls_tokens[:args.prob_word_num]
        # if len(cls_tokens) < n_gpu:
        #     cls_tokens *= n_gpu
        global_file.attended_token_num.append(len(cls_tokens))
        cls_token_ids.append(torch.tensor(tokenizer.convert_tokens_to_ids(cls_tokens)).cuda(gpu))
    if gpu == 0:
...
def update_probing_words(train_labeled_set, train_unlabeled_set, model, n_labels, epoch, tokenizer,
    ...
    savename += '_top' + str(topk) + '_noStopWords_probWrods_ep' + str(epoch) + args.prob_save_sp + '.npy'
...
def validate(valloader, model, criterion, epoch, mode, cls_token_ids=None, gpu=0):
    model.eval()
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets, length, _, _, idx) in enumerate(valloader):
            ...
            if cls_token_ids is None:
                ...
                _, outputs = model(inputs, length, probing_words_list=transform_shape(cls_token_ids, inputs.size()[0]),
...
def train(labeled_trainloader, labeled_train_iter, unlabeled_trainloader, unlabeled_train_iter, model, optimizer,
          criterion, epoch, n_labels, train_aug, cls_token_ids, gpu):
    model.train()
    ...
    for batch_idx in range(args.val_iteration):
        ...
        # if cls_token_ids is None:
        #     outputs_x = model(inputs_x, inputs_x_length, use_cls_sep=args.add_cls_sep, probing_words_list=cls_token_ids)
        # else:
        # ipdb.set_trace()
        _, prob_logits_x = model(inputs_x, inputs_x_length, use_cls_sep=args.add_cls_sep,
        ...
        _, prob_logits_aug = model(inputs_u, length_u, use_cls_sep=args.add_cls_sep,
                                   probing_words_list=transform_shape(cls_token_ids, inputs_u.size()[0]))
        _, prob_logits_aug2 = model(inputs_u2, length_u2, use_cls_sep=args.add_cls_sep,
                                    probing_words_list=transform_shape(cls_token_ids, inputs_u2.size()[0]))
        with torch.no_grad():
            # if cls_token_ids is None:
            _, prob_logits_orig = model(inputs_ori, length_ori, use_cls_sep=args.add_cls_sep,
                                        probing_words_list=transform_shape(cls_token_ids, inputs_ori.size()[0]))
            # prob_logits_orig = (prob_logits_orig + prob_logits_aug.detach() + prob_logits_aug2.detach()) / 3
            orig_y_pred = prob_logits_orig / args.T
            ...
            if prob_mask.sum() > 0:
                prob_acc = (pl_acc * prob_mask).sum() / prob_mask.sum()
            else:
                prob_acc = 0
            if gpu == 0:
                writer.add_scalar('monitor/prob_acc', prob_acc, batch_idx + args.val_iteration * epoch)

        loss = criterion(outputs_x=prob_logits_x, targets_x=targets_x, prob_logits_x=prob_logits_x,
                         outputs_u=(prob_logits_aug, prob_logits_aug2), targets_u=prob_logits_orig,
...
def eval_acc(model, unlabeled_trainloader, epoch, cls_token_ids):
    pl_acc, prob_acc = 0, 0
    pl_prob_acc = 0
    pl_prob_num = 0
    sample_num = 0
    ...
    prob_list = list()
    for batch_idx, (
            (inputs_u, inputs_u2, inputs_ori), (length_u, length_u2, length_ori), idx_u, targets_u_ori) in enumerate(
        ...
        with torch.no_grad():
            ...
            # pl_acc += (pl_pred == targets_u_ori).float().sum()
            # pl_u_idx_match = prob_logits_orig.argmax(1)
            # prob_acc += (pl_u_idx_match == targets_u_ori).float().sum()
            # sample_num += inputs_ori.shape[0]
            # pl_prob_same = (pl_pred == pl_u_idx_match).float()
            # pl_prob_num += pl_prob_same.sum()
            # pl_prob_acc += (pl_prob_same * (pl_pred == targets_u_ori).float()).sum()

            pl_list.append(orig_y_pred)
            prob_list.append(prob_logits_orig)
    # writer.add_scalar('monitor/pl_acc', pl_acc/sample_num, epoch)
    # writer.add_scalar('monitor/prob_acc', prob_acc/sample_num, epoch)
    # writer.add_scalar('monitor/pl_prob_acc', pl_prob_acc / pl_prob_num, epoch)
    # ipdb.set_trace()
    pl_all = torch.cat(pl_list, 0)
    prob_all = torch.cat(prob_list, 0)
    return pl_all, prob_all
    ...
    def __call__(self, outputs_x, targets_x, prob_logits_x, outputs_u=None, targets_u=None, prob_logits_u=None,
        ...
        if loss_u_mask.sum() > 0:
            ...
            if loss_type == '1-vs-all':
                ...
                loss_prob_u1 = self.bce_loss(self.sigmoid(prob_logits_u1.view(-1)), self.sigmoid(targets_u.view(-1)))
                loss_prob_u1 = (loss_prob_u1 * loss_prob_mask).sum() / loss_prob_mask.sum()
                loss_prob_u2 = self.bce_loss(self.sigmoid(prob_logits_u2.view(-1)), self.sigmoid(targets_u.view(-1)))
...
if __name__ == '__main__':
    mp.spawn(main, nprocs=n_gpu, join=True)
    # main()
