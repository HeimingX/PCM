...
def main():
    ...
    if args.eval:
        ...
        test_loss, test_acc, test_acc_prob, cons_ratio = validate(test_loader, model, ce_criterion, epoch,
                                                                  mode='Test Stats ',
                                                                  cls_token_ids=cls_token_ids, test=True)
        logger.info(
...
def validate(valloader, model, criterion, epoch, mode, cls_token_ids=None, test=False):
    model.eval()
    ...
    with torch.no_grad():
        ...
        correct_prob = 0
        if not test:
            num_sample_per_label = 2000
        else:
            if 'yahoo' in args.data_path:
                num_sample_per_label = 6000
            elif 'imdb' in args.data_path:
                num_sample_per_label = 12500
            elif 'dbp' in args.data_path:
                num_sample_per_label = 5000
            elif 'ag' in args.data_path:
                num_sample_per_label = 1900
            else:
                raise LookupError
        _total_sample = args.n_labels * num_sample_per_label
    ...
    whole_mask = sem_mask * match_mask * same_vote_mask
