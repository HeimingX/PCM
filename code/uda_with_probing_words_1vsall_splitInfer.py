...
parser.add_argument('--feed-prob', action="store_true", default=False)
...
if args.feed_prob:
    ...
    from probing_words_bert_noInfer import ClassificationBertWithProbingWord
...
def main():
    ...
    if args.eval:
        ...
        # cls_token_ids = None
        val_loss, val_acc, dist = validate(val_loader, model, ce_criterion, epoch, mode='Valid Stats',
                                           cls_token_ids=None)
        prediction_set, target_set = dist
...
def init_probing_words(tokenizer):
        ...
        if args.prob_file_name == 'cls_names':
            ...
            cls_wise_attended_tokens = [
                ...
                ['wonderful', 'excellent', 'fine', 'fantastic', 'perfect', 'superb', 'beautiful', 'terrific', 'amazing',
                 ...
                 'production', 'flair', 'extremely', 'recommend', 'pass', 'fair', 'pleasure', 'favorites', 'expects',
                 'spectacular', 'beauty', 'power', 'stellar', 'straightforward', 'thanking', 'luck', 'elegance',
                 'laughs', 'adapting', 'awards', 'real', 'satisfying', 'polished']]
    ...
    for cls_tokens in cls_wise_attended_tokens:
        ...
        cur_ids = list()
        cur_ids.append(torch.tensor(tokenizer.convert_tokens_to_ids(cls_tokens)).cuda())
        cur_ids.append(torch.tensor(tokenizer.convert_tokens_to_ids(['[SEP]'])).cuda())
        cls_token_ids.append(cur_ids)
...
def validate(valloader, model, criterion, epoch, mode, cls_token_ids=None):
    ...
    return loss_total, acc_total, (prediction_set, target_set)
...
def train(labeled_trainloader, labeled_train_iter, unlabeled_trainloader, unlabeled_train_iter, model, optimizer,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        prob_match_logits = list()
        for cls_id in range(n_labels):
            _, prob_logits_all = model(all_inputs, all_lengths, use_cls_sep=args.add_cls_sep, split_flag=True,
                                       probing_words_list=transform_shape(cls_token_ids[cls_id], all_inputs.size()[0]))
            prob_match_logits.append(prob_logits_all)

        prob_logits_x = torch.cat((prob_match_logits[0][:args.batch_size].unsqueeze(-1),
                                   prob_match_logits[1][:args.batch_size].unsqueeze(-1)), 1)
        prob_logits_aug = torch.cat(
            (prob_match_logits[0][args.batch_size:args.batch_size + args.batch_size_u].unsqueeze(-1),
             prob_match_logits[1][args.batch_size:args.batch_size + args.batch_size_u].unsqueeze(-1)), 1)
        prob_logits_aug2 = torch.cat((prob_match_logits[0][-args.batch_size_u:].unsqueeze(-1),
                                      prob_match_logits[1][-args.batch_size_u:].unsqueeze(-1)), 1)
        ...
        with torch.no_grad():
            orig_y_pred = model(inputs_ori, length_ori, use_cls_sep=args.add_cls_sep,
                                probing_words_list=None)
            prob_match_logits_orig = list()
            for cls_id in range(n_labels):
                _, _prob_logits_orig = model(inputs_ori, length_ori, use_cls_sep=args.add_cls_sep, split_flag=True,
                                             probing_words_list=transform_shape(cls_token_ids[cls_id],
                                                                                inputs_ori.size()[0]))
                prob_match_logits_orig.append(_prob_logits_orig)
            prob_logits_orig = torch.cat((prob_match_logits_orig[0].unsqueeze(-1),
                                          prob_match_logits_orig[1].unsqueeze(-1)), 1)
        ...
        if batch_idx % 100 == 0:
            logger.info(
                'epoch {}-{}, loss: total {}, l {}, ul {}, lprob {}, uprob {}, mask {} - {}'.format(epoch, batch_idx,
                                                                                                    ...
                                                                                                    loss_u_mask.mean(),
                                                                                                    pl_acc))
            # print('label: max softmax:{}, \n max sigmoid:{}'.format(torch.max(torch.softmax(outputs_x, dim=1), 1)[0],
            #                                                         torch.max(torch.sigmoid(prob_logits_x), 1)[0]))
            # print('ul: max softmax:{}, \n max sigmoid:{}'.format(pl_u, pl_u_match))
