...
parser.add_argument('--fix_ep', default=2, type=int, help='fix epoch for updating CSR')
...
def main():
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        val_loss, val_acc, val_acc_prob = validate(val_loader, model, ce_criterion, epoch, mode='Valid Stats',
                                                   cls_token_ids=cls_token_ids)
        logger.info("epoch {}, val acc {}, val_loss {}, prob acc {}".format(epoch, val_acc, val_loss, val_acc_prob))
        ...
        if args.prob_word_type == 'dynamic' and (epoch+1) % args.fix_ep == 0:
...
def validate(valloader, model, criterion, epoch, mode, cls_token_ids=None):
    ...
    return loss_total, acc_total, acc_prob
