...
def train(labeled_trainloader, labeled_train_iter, unlabeled_trainloader, unlabeled_train_iter, model, optimizer,
    ...
    for batch_idx in range(args.val_iteration):
        ...
        loss = criterion(outputs_x=outputs_x, targets_x=targets_x, prob_logits_x=prob_logits_x,
                         outputs_u=(aug_y_pred, aug_y_pred2), targets_u=(orig_y_pred, prob_proba_orig),
        ...
        if batch_idx % 100 == 0:
            logger.info(
                'epoch {}-{}, loss: total {}, l {}, ul {}, lprob {}, uprob {}, mask {} - {}'.format(epoch, batch_idx,
                                                                                                    ...
                                                                                                    loss_u_mask.mean(),
                                                                                                    pl_acc))
            print('label: max softmax:{}'.format(torch.max(torch.softmax(outputs_x, dim=1), 1)[0]))
...
class SemiLoss(object):
    ...
    def __call__(self, outputs_x, targets_x, prob_logits_x, outputs_u=None, targets_u=None, prob_logits_u=None,
        ...
        if loss_u_mask.sum() > 0:
            ...
            #     loss_prob_u1 = self.bce_loss(self.sigmoid(prob_logits_u1.view(-1)), targets_u_prob.view(-1))
            #     loss_prob_u1 = (loss_prob_u1 * loss_prob_mask).sum() / loss_prob_mask.sum()
            #     loss_prob_u2 = self.bce_loss(self.sigmoid(prob_logits_u2.view(-1)), targets_u_prob.view(-1))
