...
class ClassificationBertWithProbingWord(nn.Module):
    ...
    def forward(self, token_id, text_len, probing_words_list=None, use_cls_sep=True, output_attention=False, split_flag=False):
        ...
        if probing_words_list is None:
            ...
            for cls_id in range(added_len-1):
                ...
                cos_dist_list.append(cos(seq_gap_fea, out[0][:, cls_id-added_len]))
                prob_head_logits.append(self.prob_match_cls(cur_match_fea))
            cos_dist = torch.stack(cos_dist_list, 1)
            cos_dist_prob = torch.softmax(cos_dist/0.1, 1)
            #ipdb.set_trace()
